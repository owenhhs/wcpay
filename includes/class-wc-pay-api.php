 ...
 * API class
 *
 * @package WooCommerce_pay/Classes/API
 * @version 2.12.0
 */
...
class WC_pay_API {
     ...
     * Constructor.
     *
     * @param WC_pay_Gateway $gateway Payment Gateway instance.
     */
    ...
    protected function get_checkout_url() {
        return 'https://ws.' . $this->get_environment() . 'pay.uol.com.br/v2/checkout';
    }
    ...
    protected function get_sessions_url() {
        return 'https://ws.' . $this->get_environment() . 'pay.uol.com.br/v2/sessions';
    }
    ...
    protected function get_payment_url( $token ) {
        return 'https://' . $this->get_environment() . 'pay.uol.com.br/v2/checkout/payment.html?code=' . $token;
    }
    ...
    protected function get_transactions_url() {
        return 'https://ws.' . $this->get_environment() . 'pay.uol.com.br/v2/transactions';
    }
    ...
    public function get_lightbox_url() {
        return 'https://stc.' . $this->get_environment() . 'pay.uol.com.br/pay/api/v2/checkout/pay.lightbox.js';
    }
    ...
    public function get_direct_payment_url() {
        return 'https://stc.' . $this->get_environment() . 'pay.uol.com.br/pay/api/v2/checkout/pay.directpayment.js';
    }
    ...
    protected function get_notification_url() {
        return 'https://ws.' . $this->get_environment() . 'pay.uol.com.br/v2/transactions/notifications/';
    }
    ...
    public function get_payment_name_by_type( $value ) {
        $types = array(
            1 => __( 'Credit Card', 'woocommerce-pay' ),
            2 => __( 'Billet', 'woocommerce-pay' ),
            3 => __( 'Bank Transfer', 'woocommerce-pay' ),
            4 => __( 'pay credit', 'woocommerce-pay' ),
            5 => __( 'Oi Paggo', 'woocommerce-pay' ),
            7 => __( 'Account deposit', 'woocommerce-pay' ),
        );

        return isset( $types[ $value ] ) ? $types[ $value ] : __( 'Unknown', 'woocommerce-pay' );
    }
    ...
    public function get_payment_method_name( $value ) {
        $credit = __( 'Credit Card %s', 'woocommerce-pay' );
        $ticket = __( 'Billet %s', 'woocommerce-pay' );
        $debit  = __( 'Bank Transfer %s', 'woocommerce-pay' );
        ...
        $methods = array(
            ...
            401 => __( 'pay credit', 'woocommerce-pay' ),
            501 => __( 'Oi Paggo', 'woocommerce-pay' ),
            701 => __( 'Account deposit', 'woocommerce-pay' ),
        );

        return isset( $methods[ $value ] ) ? $methods[ $value ] : __( 'Unknown', 'woocommerce-pay' );
    }
    ...
    public function get_error_message( $code ) {
        ...
        $messages = array(
            '11013' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '11014' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '53018' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '53019' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '53020' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '53021' => __( 'Please enter with a valid phone number with DDD. Example: (11) 5555-5555.', 'woocommerce-pay' ),
            '11017' => __( 'Please enter with a valid zip code number.', 'woocommerce-pay' ),
            '53022' => __( 'Please enter with a valid zip code number.', 'woocommerce-pay' ),
            '53023' => __( 'Please enter with a valid zip code number.', 'woocommerce-pay' ),
            '53053' => __( 'Please enter with a valid zip code number.', 'woocommerce-pay' ),
            '53054' => __( 'Please enter with a valid zip code number.', 'woocommerce-pay' ),
            '11164' => __( 'Please enter with a valid CPF number.', 'woocommerce-pay' ),
            '53110' => '',
            '53111' => __( 'Please select a bank to make payment by bank transfer.', 'woocommerce-pay' ),
            '53045' => __( 'Credit card holder CPF is required.', 'woocommerce-pay' ),
            '53047' => __( 'Credit card holder birthdate is required.', 'woocommerce-pay' ),
            '53042' => __( 'Credit card holder name is required.', 'woocommerce-pay' ),
            '53049' => __( 'Credit card holder phone is required.', 'woocommerce-pay' ),
            '53051' => __( 'Credit card holder phone is required.', 'woocommerce-pay' ),
            '11020' => __( 'The address complement is too long, it cannot be more than 40 characters.', 'woocommerce-pay' ),
            '53028' => __( 'The address complement is too long, it cannot be more than 40 characters.', 'woocommerce-pay' ),
            '53029' => __( '<strong>Neighborhood</strong> is a required field.', 'woocommerce-pay' ),
            '53046' => __( 'Credit card holder CPF invalid.', 'woocommerce-pay' ),
            '53122' => __( 'Invalid email domain. You must use an email @sandbox.pay.com.br while you are using the pay Sandbox.', 'woocommerce-pay' ),
            '53081' => __( 'The customer email can not be the same as the pay account owner.', 'woocommerce-pay' ),
        );
        ...
        return __( 'An error has occurred while processing your payment, please review your data and try again. Or contact us for assistance.', 'woocommerce-pay' );
    }
    ...
    protected function get_order_items( $order ) {
        ...
        if ( 'yes' == $this->gateway->send_only_total ) {
            $items[] = array(
                'description' => $this->sanitize_description( sprintf( __( 'Order %s', 'woocommerce-pay' ), $order->get_order_number() ) ),
    ...
    protected function get_checkout_xml( $order, $posted ) {
        ...
        $xml = new WC_pay_XML( '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><checkout></checkout>' );
        ...
        // Checks if is localhost... pay not accept localhost urls!
        if ( ! in_array( $this->is_localhost(), array( 'localhost', '127.0.0.1' ) ) ) {
            $xml->add_redirect_url( $this->gateway->get_return_url( $order ) );
            $xml->add_notification_url( WC()->api_request_url( 'WC_pay_Gateway' ) );
        }
        ...
        // Filter the XML.
        $xml = apply_filters( 'woocommerce_pay_checkout_xml', $xml, $order );
    ...
    protected function get_payment_xml( $order, $posted ) {
        $data    = $this->get_order_items( $order );
        $ship_to = isset( $posted['ship_to_different_address'] ) ? true : false;
        $method  = isset( $posted['pay_payment_method'] ) ? $this->get_payment_method( $posted['pay_payment_method'] ) : '';
        $hash    = isset( $posted['pay_sender_hash'] ) ? sanitize_text_field( $posted['pay_sender_hash'] ) : '';

        // Creates the payment xml.
        $xml = new WC_pay_XML( '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><payment></payment>' );
        ...
        if ( 'pay' == $method ) {
            $credit_card_token = isset( $posted['pay_credit_card_hash'] ) ? sanitize_text_field( $posted['pay_credit_card_hash'] ) : '';
            $installment       = array(
                'quantity' => isset( $posted['pay_card_installments'] ) ? absint( $posted['pay_card_installments'] ) : '',
                'value'    => isset( $posted['pay_installment_value'] ) ? $this->money_format( $posted['pay_installment_value'] ) : '',
            );
            $holder_data       = array(
                'name'       => isset( $posted['pay_card_holder_name'] ) ? sanitize_text_field( $posted['pay_card_holder_name'] ) : '',
                'cpf'        => isset( $posted['pay_card_holder_cpf'] ) ? sanitize_text_field( $posted['pay_card_holder_cpf'] ) : '',
                'birth_date' => isset( $posted['pay_card_holder_birth_date'] ) ? sanitize_text_field( $posted['pay_card_holder_birth_date'] ) : '',
                'phone'      => isset( $posted['pay_card_holder_phone'] ) ? sanitize_text_field( $posted['pay_card_holder_phone'] ) : '',
            );
            ...
            $bank_name = isset( $posted['pay_bank_transfer'] ) ? sanitize_text_field( $posted['pay_bank_transfer'] ) : '';
            $xml->add_bank_data( $bank_name );
        }

        // Filter the XML.
        $xml = apply_filters( 'woocommerce_pay_payment_xml', $xml, $order );
    ...
    public function do_checkout_request( $order, $posted ) {
        ...
        if ( 'yes' == $this->gateway->debug ) {
            $this->gateway->log->add( $this->gateway->id, 'Error generating the pay payment token: ' . print_r( $response, true ) );
        }
    ...
    public function do_payment_request( $order, $posted ) {
        $payment_method = isset( $posted['pay_payment_method'] ) ? $posted['pay_payment_method'] : '';
        ...
        if ( ! in_array( $payment_method, $this->get_available_payment_methods() ) ) {
            return array(
                'url'   => '',
                'data'  => '',
                'error' => array( '<strong>' . __( 'pay', 'woocommerce-pay' ) . '</strong>: ' .  __( 'Please, select a payment method.', 'woocommerce-pay' ) ),
            );
        }
        ...
        if ( is_wp_error( $response ) ) {
            ...
            if ( 'yes' == $this->gateway->debug ) {
                $this->gateway->log->add( $this->gateway->id, 'The user does not have permissions to use the pay Transparent Checkout!' );
            }

            return array(
                'url'   => '',
                'data'  => '',
                'error' => array( __( 'You are not allowed to use the pay Transparent Checkout. Looks like you neglected to installation guide of this plugin. This is not pretty, do you know?', 'woocommerce-pay' ) ),
            );
        } else {
            try {
                ...
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'Error while parsing the pay response: ' . print_r( $e->getMessage(), true ) );
                }
            }

            if ( isset( $data->code ) ) {
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'pay direct payment created successfully!' );
                }
            ...
            if ( isset( $data->error ) ) {
                ...
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'An error occurred while generating the pay direct payment: ' . print_r( $response, true ) );
                }

                foreach ( $data->error as $error_key => $error ) {
                    if ( $message = $this->get_error_message( $error->code ) ) {
                        $errors[] = '<strong>' . __( 'pay', 'woocommerce-pay' ) . '</strong>: ' . $message;
                    }
                }
        ...
        return array(
            'url'   => '',
            'data'  => '',
            'error' => array( '<strong>' . __( 'pay', 'woocommerce-pay' ) . '</strong>: ' . __( 'An error has occurred while processing your payment, please try again. Or contact us for assistance.', 'woocommerce-pay' ) ),
        );
    }
    ...
    public function process_ipn_request( $data ) {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            try {
                ...
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'Error while parsing the pay IPN response: ' . print_r( $e->getMessage(), true ) );
                }
            }

            if ( isset( $body->code ) ) {
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'pay IPN is valid! The return is: ' . print_r( $body, true ) );
                }
    ...
    public function get_session_id() {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            try {
                ...
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'Error while parsing the pay session response: ' . print_r( $e->getMessage(), true ) );
                }
            }

            if ( isset( $session->id ) ) {
                if ( 'yes' == $this->gateway->debug ) {
                    $this->gateway->log->add( $this->gateway->id, 'pay session is valid! The return is: ' . print_r( $session, true ) );
                }
